...
- **Bulk shipment import**: new `lvt-cert import-shipments` CLI and `POST /api/admin/shipments/import` endpoint parse carrier Excel/CSV exports into a new `shipment_history` table (PK `(round_id, sbd, tracking_code)`, so the audit trail is preserved). Config-driven multi-carrier profiles (`features.shipment.import.profiles.{viettel, ghn, ...}`), column-name fallbacks, `success_keywords` per carrier, `--dry-run` default with `--commit` opt-in. SBD matching via `data_mapping.phone_col`. Admin-only API endpoint with 10 MB cap + 5 req/min rate limit. Logs `shipment.bulk_import` with summary metadata (no PII).
...
- **`X-Forwarded-For` opt-in**: trusted only when `TRUST_PROXY_HEADERS=1`. Previously trusted unconditionally, so clients could spoof the header to bypass rate limits. (H5)
...
- **`admin.student.update` audit metadata** no longer persists raw `old`/`new` values; records `{column, changed, value_length_delta}` only, so PII never reaches the GSheet forward. (M5)
...
- Admin subsystem: PBKDF2 passwords, JWT HS256 tokens (strict `JWT_SECRET` with no ephemeral fallback), three login flows (password / OTP-email / magic-link) with uniform-time responses for unknown emails, `admin_activity` audit log with optional async GSheet webhook.
- Opt-in shipment tracking with `(round_id, sbd)` upserts, race-safe `ON CONFLICT DO UPDATE`, and `public_fields` allowlist (default empty, so students see only `status` + `updated_at`).
